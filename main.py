...
try:
    ...
    # 10년 이동평균 추세선 계산
    yearly_df['10년_이동평균'] = yearly_df['연평균기온'].rolling(window=10, min_periods=1).mean().round(2)
    
    # 주요 지표 요약 카드
    col1, col2, col3, col4 = st.columns(4)
    
    min_year_row = yearly_df.loc[yearly_df['연평균기온'].idxmin()]
    max_year_row = yearly_df.loc[yearly_df['연평균기온'].idxmax()]
    first_year_avg = yearly_df.iloc[0]['연평균기온']
    last_year_avg = yearly_df.iloc[-1]['연평균기온']
    ...
    with col3:
        st.metric(
            label="분석 기간",
            value=f"{int(yearly_df['연도'].min())}년 ~ {int(yearly_df['연도'].max())}년",
            delta=f"총 {len(yearly_df)}개 연도"
        )
    with col4:
        st.metric(
            label="관측 시작 대비 변화",
    ...
    with col_left:
        ...
        fig_sub = go.Figure()
        fig_sub.add_trace(go.Scatter(x=yearly_df['연도'], y=yearly_df['연평균최고기온'], name='연평균 최고기온', line=dict(color='#E53935')))
        fig_sub.add_trace(go.Scatter(x=yearly_df['연도'], y=yearly_df['연평균최저기온'], name='연평균 최저기온', line=dict(color='#1E88E5')))
